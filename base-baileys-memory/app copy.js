...
/* const correo = addKeyword("1").addAnswer(
  "Digita tu correo electronicco",
  { capture: true },
  async (ctx, { flowDynamic, fallBack }) => {
    if (validCorreo.test(ctx.body)) {
      GLOBAL_STATE[ctx.from].correo = ctx.body;
      guardarInfo(GLOBAL_STATE[ctx.from]);
      await flowDynamic("Hemos guardado tu informacion");
    } else {
 ...
 */
/* Sacar numero  */
const informacion = addKeyword(["Info", "informacion"]).addAnswer(
  "Obteniendo tu numero",
  null,
  async (ctx, { flowDynamic }) => {
    console.log("Numero: " + ctx.from);
    cel = ctx.from;
    await flowDynamic([{ body: `Tu numero es: *${cel}*` }]);
  }
);
  ...
  .addAnswer(
   ...
   /*    
    ...
    const sentMsg  = await sock.sendMessage(id, { text: 'Hi, this was sent using https://github.com/adiwajshing/baileys' }) */
     /* ENVIO TARJETA DE CONTACTO
     const vcard =
        "BEGIN:VCARD\n" + // metadata of the contact card
        "VERSION:3.0\n" +
        "FN:Cristian Garcia\n" + // full name
        "ORG:Nacional de electricos;\n" + // the organization of the contact
        "TEL;type=CELL;type=VOICE;waid=573168108421:+57 31681 08421\n" + // WhatsApp ID + phone number
        "END:VCARD";
...
const img = addKeyword("img").addAnswer("HOli", {
  media:
    "https://www.lavanguardia.com/files/image_449_220/files/fp/uploads/2022/10/20/63514db4cf05f.r_d.1463-984-1497.jpeg",
});

const noteVoice = addKeyword(EVENTS.VOICE_NOTE).addAnswer(
  "Te escucho en un momento 👂🏻"
);
/* Callback function */
let cel;
...
const main = async () => {
  ...
  const adapterFlow = createFlow([inicioChat, noteVoice, img]);
